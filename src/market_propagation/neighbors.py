...
#: Where a contract was observed. Membership in the study population is decided by
#: the eligibility predicate plus at least one observation path, so these name the
#: *paths* rather than the admission rule: a contract may be eligible and observed
#: through either one, or through both. The distinction is kept rather than collapsed
#: because the two paths are different evidence, and a consumer has to be able to
#: condition on it without the population being redefined after the fact.
ORIGIN_ARCHIVED_ONLY = "archived_only"
ORIGIN_LIVE_ONLY = "live_only"
ORIGIN_ARCHIVED_AND_LIVE = "archived_and_live"

#: A contract generated by the declared synthetic process. It is not a member of the
#: study population and is not observed through either real path, so it states that
#: rather than borrowing a provenance it does not have.
...
def observation_origin(*, seen_archive: bool, seen_live: bool) -> str:
    """The origin of a contract observed through the paths it was seen on.

    A contract observed through neither path has no origin to state, and is refused
    rather than defaulted to one: an unobserved contract is not an archived one, and
    writing the default would admit it to the population on a provenance nobody
    observed. ``simulated`` is deliberately unreachable here — it is not an
    observation at all.
    """
    if seen_archive and seen_live:
        return ORIGIN_ARCHIVED_AND_LIVE
    if seen_live:
        return ORIGIN_LIVE_ONLY
    if seen_archive:
        return ORIGIN_ARCHIVED_ONLY
    raise ValueError(
        "a contract observed through neither the archive nor the live path has no "
        "observation origin; one of the two must have observed it"
    )
...
class ContractPredicate:
    ...
    def __post_init__(self) -> None:
        ...
        if self.observation_origin not in OBSERVATION_ORIGINS:
            raise ValueError(
                f"ContractPredicate.observation_origin must be one of {OBSERVATION_ORIGINS}, "
                f"got {self.observation_origin!r}; a contract whose provenance is not a known "
                "observation path cannot be admitted on it"
            )
        object.__setattr__(
